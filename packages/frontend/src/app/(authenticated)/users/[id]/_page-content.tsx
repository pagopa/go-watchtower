...
import { ArrowLeft, Pencil, Trash2, X, CalendarDays, ShieldCheck } from 'lucide-react'
...
function UserHero({
  ...
  return (
    <div className="relative overflow-hidden rounded-xl border bg-card">
      <div className={cn('absolute left-0 top-0 h-full w-1', user.isActive ? 'bg-success' : 'bg-muted-foreground/25')} />
      <div className="py-5 pl-7 pr-6">
        <div className="flex items-start justify-between gap-4">
          <div className="flex min-w-0 flex-1 items-start gap-4">
            <UserAvatar name={user.name} />
            <div className="min-w-0">
              <div className="mb-1 flex items-center gap-2">
                <div className={cn('h-1.5 w-1.5 rounded-full', user.isActive ? 'bg-success' : 'bg-muted-foreground/40')} />
                <span className={cn('text-xs font-semibold uppercase tracking-widest', user.isActive ? 'text-success' : 'text-muted-foreground')}>
            ...
            <div className="flex shrink-0 gap-2">
              {canWrite && (
                <Button variant="outline" size="sm" asChild>
                  <Link href={`/users/${userId}/edit`}><Pencil className="mr-1.5 h-3.5 w-3.5" />Modifica</Link>
                </Button>
              )}
              {canDelete && (
        ...
        <dl className="mt-4 grid grid-cols-2 gap-x-8 gap-y-3 border-t pt-4 sm:grid-cols-4">
          <div>
            <dt className="text-xs font-medium uppercase tracking-widest text-muted-foreground">Provider</dt>
            <dd className="mt-1.5"><Badge variant="secondary" className="font-mono text-xs">{user.provider}</Badge></dd>
          </div>
          <div>
            <dt className="text-xs font-medium uppercase tracking-widest text-muted-foreground">Ruolo</dt>
            <dd className="mt-1.5">
              {canWrite ? (
                <Select value={currentRole?.id} onValueChange={onRoleChange} disabled={roleChangePending}>
          ...
          <div>
            <dt className="flex items-center gap-1 text-xs font-medium uppercase tracking-widest text-muted-foreground"><CalendarDays className="h-3 w-3" />Creato</dt>
            <dd className="mt-1.5 text-sm tabular-nums">{formatDate(user.createdAt)}</dd>
          </div>
          <div>
            <dt className="flex items-center gap-1 text-xs font-medium uppercase tracking-widest text-muted-foreground"><CalendarDays className="h-3 w-3" />Aggiornato</dt>
...
function UserPermissionsPanel({
  ...
  getOverride: (resource: string) => UserPermissionOverride | null
  getRolePermission: (resource: string, action: PermAction) => PermissionScope
  getOverrideValue: (resource: string, action: PermAction) => PermissionScope | null
  onOverrideChange: (resource: string, action: PermAction, value: OverrideSelectValue) => void
  onRemoveOverride: (resource: string) => void
}) {
  return (
    <div className="overflow-hidden rounded-xl border bg-card">
      <div className="flex items-center justify-between border-b px-6 py-4">
        <div>
          <h2 className="flex items-center gap-2 font-semibold"><ShieldCheck className="h-4 w-4 text-muted-foreground" />Permessi</h2>
          <p className="mt-0.5 text-sm text-muted-foreground">
            Valori da ruolo in grigio &middot; override specifici in colore
            {overrideCount > 0 && (
              <span className="ml-2 inline-flex items-center rounded-full bg-primary/10 px-2 py-0.5 text-xs font-medium text-primary">{overrideCount} override</span>
            )}
          </p>
        </div>
      </div>
        ...
        <>
          <div className="flex items-center gap-4 bg-muted/30 px-6 py-2 text-xs font-medium uppercase tracking-widest text-muted-foreground">
            ...
            <div className="flex flex-1 items-center">
              <div className="flex-1 text-center">Lettura</div><div className="flex-1 text-center">Scrittura</div><div className="flex-1 text-center">Eliminazione</div>
            </div>
          ...
          <div className="divide-y">
            {DISPLAY_RESOURCES.map((resource) => {
              ...
              return (
                <div
                  key={resource}
                  className={cn('relative flex items-center gap-4 px-6 py-3 transition-colors', hasOverride ? 'bg-primary/[0.025]' : 'hover:bg-muted/20')}
                >
                  {hasOverride && <div className="absolute left-0 top-0 h-full w-0.5 bg-primary/50" />}
                  <div className="w-40 shrink-0 pl-3">
                    <span className="text-sm font-medium">{RESOURCE_LABELS[resource] ?? resource}</span>
                    {hasOverride && <div className="mt-0.5 text-[10px] font-medium uppercase tracking-wider text-primary/70">override</div>}
                  </div>
                  <div className="flex flex-1 items-center">
                        ...
                        <div key={action} className="flex flex-1 flex-col items-center gap-1.5">
                          {canWrite ? (
                            <Select
                              ...
                              onValueChange={(value) => onOverrideChange(resource, action, value as OverrideSelectValue)}
                              disabled={overridePending || removePending}
                            >
                              <SelectTrigger className={cn('h-7 w-[120px] text-xs', hasActionOverride && 'border-primary/30')}><SelectValue /></SelectTrigger>
                              <SelectContent>
                          ...
                          <span className="text-[10px] tabular-nums text-muted-foreground/60">ruolo: {PERMISSION_SCOPE_LABELS[roleValue]}</span>
                        </div>
                      )
                    })}
                  </div>
                    ...
                    <div className="flex w-8 shrink-0 justify-center">
                      {hasOverride && (
                        <Button
                          ...
                          onClick={() => onRemoveOverride(resource)}
  ...
  return (
    <div className="space-y-5">
      ...
      <UserHero
        ...
        onRoleChange={(roleId) => changeRoleMutation.mutate(roleId)}
      ...
      <UserPermissionsPanel
        ...
        overridePending={setOverrideMutation.isPending}
        removePending={removeOverrideMutation.isPending}
        getOverride={getOverride}
        getRolePermission={getRolePermission}
        getOverrideValue={getOverrideValue}
        onOverrideChange={handleOverrideChange}
        onRemoveOverride={(resource) => removeOverrideMutation.mutate(resource)}
      />
