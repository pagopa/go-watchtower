...
const AnalysisFormDialog = dynamic(
  () =>
    import("./_components/analysis-form-dialog").then((m) => ({
      default: m.AnalysisFormDialog,
    })),
  { ssr: false },
);
...
const ShortcutInCorsoDialog = dynamic(
  () =>
    import("./_components/shortcut-in-corso-dialog").then((m) => ({
      default: m.ShortcutInCorsoDialog,
    })),
  { ssr: false },
);
...
const ShortcutIgnorableDialog = dynamic(
  () =>
    import("./_components/shortcut-ignorable-dialog").then((m) => ({
      default: m.ShortcutIgnorableDialog,
    })),
  { ssr: false },
);
...
const AnalysisDailyView = dynamic(
  () =>
    import("./_components/analysis-daily-view").then((m) => ({
      default: m.AnalysisDailyView,
    })),
  { ssr: false },
);
...
const AnalysisOnCallView = dynamic(
  () =>
    import("./_components/analysis-oncall-view").then((m) => ({
      default: m.AnalysisOnCallView,
    })),
  { ssr: false },
);
...
function AnalysesPageContent() {
  ...
  const [filterOverrides, setFilterOverrides] = useState<
    Record<string, AnalysisFiltersState>
  >({});
  ...
  const [viewModeOverride, setViewModeOverride] = useState<
    "list" | "daily" | "oncall" | null
  >(null);
  const viewMode =
    viewModeOverride ??
    (preferences.analysisViewMode as "list" | "daily" | "oncall" | undefined) ??
    "list";

  const handleSetViewMode = useCallback(
    (mode: "list" | "daily" | "oncall") => {
  ...
  const [selectedAnalysis, setSelectedAnalysis] =
    useState<AlarmAnalysis | null>(null);
  ...
  const [validationPanelAnalysis, setValidationPanelAnalysis] =
    useState<AlarmAnalysis | null>(null);
  ...
  const [autoOpenedAnalysisId, setAutoOpenedAnalysisId] = useState<
    string | null
  >(null);
  ...
  const { collapsed: filtersCollapsed, toggle: handleToggleFiltersCollapsed } =
    useCollapsiblePreference("analysisFiltersCollapsed");
  ...
  const futureOffsetMinutes =
    analysisPolicy?.analysisFutureOffsetMinutes ?? null;
  ...
  const isAnalysisLocked = useCallback(
    (analysis: AlarmAnalysis): boolean => {
      ...
      const daysSince = Math.floor(
        (Date.now() - new Date(analysis.createdAt).getTime()) / 86_400_000,
      );
  ...
  const columnDefs = useMemo(
    () =>
      isAllView
        ? ANALYSIS_COLUMNS
        : ANALYSIS_COLUMNS.filter((c) => c.id !== "product"),
    [isAllView],
  );
  ...
  const totalTableMinWidth = useTableMinWidth(
    visibleColumns,
    getWidth,
    canWrite || canDelete,
  );
  ...
  const analysisQueryParams = useMemo(
    ...
    [
      ...
      filters.operatorIds,
      filters.alarmIds,
      filters.finalActionIds,
      filters.priorityCodes,
      filters.isOnCall,
      filters.dateFrom,
      filters.dateTo,
      filters.ignoreReasonCodes,
      filters.runbookIds,
      filters.resourceIds,
  ...
  const validationCache = useMemo(() => {
    if (!analyses)
      return new Map<
        string,
        { validation: ValidationResult; quality: QualityResult }
      >();
    return new Map(
      analyses.map((a) => [
        a.id,
  ...
  const handleFilterChange = useCallback(
    (newFilters: AnalysisFiltersState) => {
      setFilterOverrides((prev) => ({
        ...prev,
        [filterKeyRef.current]: newFilters,
      }));
      setPage(1);
  ...
  const handleResetFilters = useCallback(() => {
    ...
    setFilterOverrides((prev) => ({
      ...prev,
      [filterKeyRef.current]: { ...DEFAULT_FILTERS },
    }));
    setPage(1);
    if (saveTimerRef.current) {
      clearTimeout(saveTimerRef.current);
  ...
  const handleEdit = async (analysis: AlarmAnalysis) => {
    try {
      const detail = await queryClient.fetchQuery({
        queryKey: qk.analyses.detail(analysis.productId, analysis.id),
        queryFn: () => api.getAnalysis(analysis.productId, analysis.id),
        staleTime: 30_000,
      });
      setFormProductId(detail.productId);
      setEditItem(detail);
      setActiveShortcut("full");
    } catch {
      toast.error("Impossibile caricare il dettaglio completo dell’analisi");
    }
  };
  ...
  const handleFormSubmit = (data: AnalysisFormData) => {
    const targetProductId =
      editItem?.productId || formProductId || effectiveProductId;
    if (!targetProductId) {
            ...
            {analysesLoading ? (
              ...
              <div className="flex flex-col items-center gap-2 py-12 text-center">
                <p className="text-sm text-destructive">
                  Errore durante il caricamento delle analisi.
                </p>
              </div>
            ) : analyses && analyses.length > 0 ? (
              <Table
              ...
              <div className="flex flex-col items-center gap-4 py-16 text-center">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Prova a modificare i filtri o crea una nuova analisi.
                  </p>
                </div>
                {canWrite && (
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => handleShortcutSelect("full")}
                  >
      ...
      {viewMode === "list" && pagination && (
        <PaginationControls
          ...
          onPageSizeChange={(size) => {
            setPageSize(size as AllowedPageSize);
            setPage(1);
          }}
        />
      )}
